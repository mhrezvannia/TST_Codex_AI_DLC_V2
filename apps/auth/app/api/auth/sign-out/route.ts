...
import { authConfig, clearCookieHeader, redirectResponse } from "../../../../lib/auth-server";

export function POST(request: Request) {
  const logoutUrl = new URL(authConfig.keycloakLogoutUrl);
  logoutUrl.searchParams.set("post_logout_redirect_uri", new URL("/signed-out", request.url).toString());
