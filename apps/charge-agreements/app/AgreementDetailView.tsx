...
export function AgreementDetailView({ agreementId }: { agreementId: string }) {
  ...
  return <main className="rates-page">
    <header className="rates-page-header"><div><p className="rates-eyebrow">Agreement authority / detail</p><h1 className="rates-title">{detail.agreementNumber}</h1><p className="rates-muted">{detail.agreementId} / {detail.authorityModel}{detail.readOnly ? " / read-only" : ""}</p></div><div className="rates-actions">{detail.capabilities.canUpdate && version?.lifecycle === "DRAFT" ? <a className="rates-link" href={agreementAppPath(`/${encodeURIComponent(detail.agreementId)}/edit`)}>Edit Draft</a> : null}{detail.capabilities.canCreateSuccessor && version?.lifecycle === "APPROVED" ? <a className="rates-link" href={agreementAppPath(`/${encodeURIComponent(detail.agreementId)}/successor`)}>Create successor</a> : null}</div></header>
    ...
    <Card title="RateVersion links"><div className="rates-table-region" role="region" aria-label="RateVersion links" tabIndex={0}><Table><thead><tr><th scope="col">Category</th><th scope="col">Immutable RateVersion ID</th></tr></thead><tbody>{version?.rateLinks.map((link) => <tr key={link.category}><td>{link.category}</td><td>{link.rateVersionId}</td></tr>)}</tbody></Table></div></Card>
