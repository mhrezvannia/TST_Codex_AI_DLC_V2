...
public record AgreementOutboxEvent(
        String eventId,
        String eventType,
    ...
    public AgreementOutboxEvent retryable(String code, String message, Instant nextAttempt) {
        return copy(payload, OutboxStatus.RETRYABLE, attemptCount + 1,
                nextAttempt, null, null, code, message);
    }

    public AgreementOutboxEvent failedPermanent(String code, String message) {
        return copy(payload, OutboxStatus.FAILED_PERMANENT, attemptCount + 1,
                null, null, null, code, message);
    }

    private AgreementOutboxEvent copy(
        ...
        return new AgreementOutboxEvent(eventId, eventType, schemaVersion, agreementId, agreementStatus,
                agreementVersion, schemaSubject, producerIdentity, deduplicationKey, correlationId,
                occurredAt, nextPayload, nextStatus, nextAttemptCount, nextAttempt, nextClaimedBy,
                nextClaimedAt, nextErrorCode, nextErrorMessage);
    }
}
